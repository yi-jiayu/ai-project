...
def create_image_lists(image_dir, split_dir, zoom_level):
  ...
  Args:
    ...
    split_dir: String path to a folder containing the breakhis splits.
    zoom_level: String representing the zoom level of the images to use.
  ...
  for set_ in ('train', 'val', 'test'):
    if not gfile.Exists(os.path.join(split_dir, zoom_level + '_' + set_ + '.txt')):
      print("Could not find {} set information.".format(set_))
      not_found = True
  ...
  for set_ in ('train', 'val', 'test'):
    with open(os.path.join(split_dir, zoom_level + '_' + set_ + '.txt')) as f:
      for line in f:
        ...
        if img_label == '0':
          ...
          raise ValueError('invalid label: "{}"'.format(img_label))

        if set_ == 'train':
          result[label_str]['training'].append(img_path)
        elif set_ == 'val':
          result[label_str]['validation'].append(img_path)
        else:
          result[label_str]['testing'].append(img_path)
...
def main(_):
  ...
  image_lists = create_image_lists(FLAGS.image_dir, FLAGS.split_dir, FLAGS.zoom_level)
  class_count = len(image_lists.keys())
  if class_count == 0:
